      ...
      <aside className={`sidebar ${open ? 'open' : ''}`}>
        ...
        <nav className="sb-nav">
          ...
          <Item active={showFavorites && page === 'contacts'}
            ...
            onClick={() => { setPage('contacts'); setShowFavorites(true); setCategory(''); setActiveTag(''); setOpen(false); }} />
          ...
          {CATS.map(cat => (
            <Item key={cat}
              ...
              onClick={() => { setPage('contacts'); setCategory(cat === category ? '' : cat); setShowFavorites(false); setActiveTag(''); setOpen(false); }} />
          ))}
          ...
          {tags?.length > 0 && <>
            ...
            {tags.slice(0, 8).map(t => (
              <Item key={t.name} active={activeTag === t.name && page === 'contacts'}
                ...
                onClick={() => { setActiveTag(t.name === activeTag ? '' : t.name); setPage('contacts'); setCategory(''); setShowFavorites(false); setOpen(false); }} />
            ))}
          </>}
