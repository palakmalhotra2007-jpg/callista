...
function Item({ active, icon, label, badge, onClick, dot, extra }) {
  return (
    <button
      className={`sb-item${active ? ' active' : ''}${extra ? ' ' + extra : ''}`}
      onClick={onClick}
    >
      {dot
    ...
    <>
      ...
      <aside className={`sidebar ${open ? 'open' : ''}`}>
        ...
        <div className="sb-search">
          ...
          />
          {search && (
            <button className="sb-search-x" onClick={() => setSearch('')}>
              <X size={11} />
            </button>
          )}
        </div>
        ...
        <nav className="sb-nav">
          ...
          <Item
            active={page === 'contacts' && !showFavorites && !category && !activeTag}
            icon={<Users size={14} />}
            label="All Contacts"
            badge={contactCount}
            onClick={() => go('contacts')}
          ...
          <Item
            active={showFavorites && page === 'contacts'}
            icon={<Star size={14} />}
            label="Favourites"
            onClick={() => {
              setPage('contacts');
          ...
          {tags?.length > 0 && (
            <>
              ...
              {tags.slice(0, 8).map(t => (
                <Item
                  key={t.name}
                  active={activeTag === t.name && page === 'contacts'}
                  icon={<Tag size={13} />}
                  label={t.name}
                  badge={t.count}
                  onClick={() => {
                    setActiveTag(t.name === activeTag ? '' : t.name);
                    setPage('contacts');
                    setCategory('');
                    setShowFavorites(false);
                    setOpen(false);
        ...
        <div className="sb-bottom">
          <div className="sb-tools">
            <button className="sb-tool" onClick={onImport}>
              <Upload size={12} /> Import CSV
            </button>
            <button className="sb-tool" onClick={onExportPDF}>
              <Download size={12} /> Export PDF
            </button>
          </div>

          <div className="sb-footer">
            <button className="sb-foot" onClick={() => go('settings')}>
              <Settings size={13} /> Settings
            </button>
            <button className="sb-foot danger" onClick={logout}>
