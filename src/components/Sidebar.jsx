...
import { Phone, Users, Star, Bell, BarChart2, Tag, LogOut, Settings, Upload, Download, X, Search, MapPin, Menu } from 'lucide-react';
...
export default function Sidebar({ page, setPage, search, setSearch, category, setCategory, showFavorites, setShowFavorites, contactCount, onImport, onExportPDF, tags, activeTag, setActiveTag }) {
  ...
  const go = p => { setPage(p); setCategory(''); setShowFavorites(false); setActiveTag(''); setOpen(false); };
    ...
    <>
      ...
      <aside className={`sidebar${open ? ' sb-open' : ''}`}>
        ...
        <div className="sb-brand">
          ...
          <button className="sb-close" onClick={close} aria-label="Close menu"><X size={16} /></button>
        </div>
        ...
        <nav className="sb-nav">
          ...
          <Item active={showFavorites && page === 'contacts'}
            ...
            onClick={() => { setPage('contacts'); setShowFavorites(true); setCategory(''); setActiveTag(''); close(); }} />
          ...
          {CATS.map(cat => (
            <Item key={cat}
              ...
              onClick={() => { setPage('contacts'); setCategory(cat === category ? '' : cat); setShowFavorites(false); setActiveTag(''); close(); }} />
          ))}
          ...
          {tags?.length > 0 && <>
            ...
            {tags.slice(0, 8).map(t => (
              <Item key={t.name} active={activeTag === t.name && page === 'contacts'}
                ...
                onClick={() => { setActiveTag(t.name === activeTag ? '' : t.name); setPage('contacts'); setCategory(''); setShowFavorites(false); close(); }} />
            ))}
          </>}
        ...
        <div className="sb-bottom">
          <div className="sb-tools">
            <button className="sb-tool" onClick={() => { onImport(); close(); }}><Upload size={12} />Import CSV</button>
            <button className="sb-tool" onClick={() => { onExportPDF(); close(); }}><Download size={12} />Export PDF</button>
          </div>
